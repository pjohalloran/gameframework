...
namespace GameHalloran
{
	...
	public:
		...
		private:
			...
			{
				colOne.SetX(m_mat[M00]);	colOne.SetY(m_mat[M01]);	colOne.SetZ(m_mat[M02]);	colOne.SetW(m_mat[M03]);
				colTwo.SetX(m_mat[M10]);	colTwo.SetY(m_mat[M11]);	colTwo.SetZ(m_mat[M12]);	colTwo.SetW(m_mat[M13]);
				colThree.SetX(m_mat[M20]);	colThree.SetY(m_mat[M21]);	colThree.SetZ(m_mat[M22]);	colThree.SetW(m_mat[M23]);
				colFour.SetX(m_mat[M30]);	colFour.SetY(m_mat[M31]);	colFour.SetZ(m_mat[M32]);	colFour.SetW(m_mat[M33]);
			}
			...
			{
				rowOne.SetX(m_mat[M00]);	rowOne.SetY(m_mat[M10]);	rowOne.SetZ(m_mat[M20]);	rowOne.SetW(m_mat[M30]);
				rowTwo.SetX(m_mat[M01]);	rowTwo.SetY(m_mat[M11]);	rowTwo.SetZ(m_mat[M21]);	rowTwo.SetW(m_mat[M31]);
				rowThree.SetX(m_mat[M02]);	rowThree.SetY(m_mat[M12]);	rowThree.SetZ(m_mat[M22]);	rowThree.SetW(m_mat[M32]);
				rowFour.SetX(m_mat[M03]);	rowFour.SetY(m_mat[M13]);	rowFour.SetZ(m_mat[M23]);	rowFour.SetW(m_mat[M33]);
			};
		...
		public:
			...
			{
				pos.SetX(m_mat[M30]);	pos.SetY(m_mat[M31]);	pos.SetZ(m_mat[M32]);	pos.SetW(m_mat[M33]);
			};
			...
			{
				m_mat[M30] = pos.GetX();	m_mat[M31] = pos.GetY();	m_mat[M32] = pos.GetZ();	m_mat[M33] = pos.GetW();
			};
			...
			{
				m_mat[M30] = pos.GetX();	m_mat[M31] = pos.GetY();	m_mat[M32] = pos.GetZ();	m_mat[M33] = 1.0f;
			};
			...
			{
				xDir.SetX(m_mat[M00]);	xDir.SetY(m_mat[M01]);	xDir.SetZ(m_mat[M02]);	xDir.SetW(m_mat[M03]);
			};
			...
			{
				m_mat[M00] = xDir.GetX();	m_mat[M01] = xDir.GetY();	m_mat[M02] = xDir.GetZ();	m_mat[M03] = xDir.GetW();
			};
			...
			{
				yDir.SetX(m_mat[M10]);	yDir.SetY(m_mat[M11]);	yDir.SetZ(m_mat[M12]);	yDir.SetW(m_mat[M13]);
			};
			...
			{
				m_mat[M10] = yDir.GetX();	m_mat[M11] = yDir.GetY();	m_mat[M12] = yDir.GetZ();	m_mat[M13] = yDir.GetW();
			};
			...
			{
				zDir.SetX(m_mat[M20]);	zDir.SetY(m_mat[M21]);	zDir.SetZ(m_mat[M22]);	zDir.SetW(m_mat[M23]);
			};
			...
			{
				m_mat[M20] = zDir.GetX();	m_mat[M21] = zDir.GetY();	m_mat[M22] = zDir.GetZ();	m_mat[M23] = zDir.GetW();
			};
