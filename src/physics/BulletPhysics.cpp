...
namespace GameHalloran
{
	...
	void BulletPhysics::SendCollisionPairAddEvent(btPersistentManifold const *manifoldPtr, btRigidBody const *body0Ptr, btRigidBody const *body1Ptr)
	{
		...
		else
		{
			...
			if(!id0.is_initialized() || !id1.is_initialized())
			{
				...
				{
					try { str = boost::lexical_cast<std::string, ActorId>(*id0); } catch(...) { }
				}
	...
	PhysicsBodyType BulletPhysics::GetPhysicsBodyType(const btCollisionObject *bodyPtr) const
	{
		...
		{
			case btCollisionObject::CO_COLLISION_OBJECT:	type = eCollisionBody;	break;
			case btCollisionObject::CO_RIGID_BODY:			type = eRigidBody;		break;
			case btCollisionObject::CO_GHOST_OBJECT:		type = eDummyBody;		break;
			case btCollisionObject::CO_SOFT_BODY:			type = eSoftBody;		break;
			default:										type = eInvalidBody;	break;
		}
