...
namespace GameHalloran {
    ...
    Matrix4::Matrix4(const Vector3 &xDir, const Vector3 &yDir, const Vector3 &zDir, const Point3 &location)
    {
        ...
        m_mat[M10] = yDir.GetX();
        m_mat[M20] = zDir.GetX();
        m_mat[M30] = location.GetX();
        m_mat[M01] = xDir.GetY();
        m_mat[M11] = yDir.GetY();
        m_mat[M21] = zDir.GetY();
        m_mat[M31] = location.GetY();
        m_mat[M02] = xDir.GetZ();
        m_mat[M12] = yDir.GetZ();
        m_mat[M22] = zDir.GetZ();
        m_mat[M32] = location.GetZ();
        m_mat[M03] = 0.0f;
        m_mat[M13] = 0.0f;
        m_mat[M23] = 0.0f;
        m_mat[M33] = 1.0f;
    ...
    // TODO: Delete this is move to header is okay..
    // /////////////////////////////////////////////////////////////////
    //
    // /////////////////////////////////////////////////////////////////
    ...
    {
        outMatrix.m_mat[M00] = m_mat[M00];
        outMatrix.m_mat[M10] = m_mat[M01];
        outMatrix.m_mat[M20] = m_mat[M02];
        outMatrix.m_mat[M30] = m_mat[M03];
        outMatrix.m_mat[M01] = m_mat[M10];
        outMatrix.m_mat[M11] = m_mat[M11];
        outMatrix.m_mat[M21] = m_mat[M12];
        outMatrix.m_mat[M31] = m_mat[M13];
        outMatrix.m_mat[M02] = m_mat[M20];
        outMatrix.m_mat[M12] = m_mat[M21];
        outMatrix.m_mat[M22] = m_mat[M22];
        outMatrix.m_mat[M32] = m_mat[M23];
        outMatrix.m_mat[M03] = m_mat[M30];
        outMatrix.m_mat[M13] = m_mat[M31];
        outMatrix.m_mat[M23] = m_mat[M32];
        outMatrix.m_mat[M33] = m_mat[M33];
    }
